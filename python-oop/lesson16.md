...
Оператор `in` (`3 in container`) работающий через последовательный перебор с помощью `__getitem__` или `__iter__` обходит весь контейнер. В худшем случае, что бы найти элемент, оператору `in` понадобится пройти по всем элементам контейнера. Это означает линейную сложность O(n), где n - это количество элементов в контейнере.
