...
Адаптер для подключения Django к PostgreSQL:

```bash
pip install psycopg2-binary
```

`psycopg2-binary` — версия с предкомпилированными бинарниками, устанавливается без компилятора и системных зависимостей. Для продакшна рекомендуют `psycopg2` (без суффикса `binary`) — он компилируется из исходников и лучше оптимизирован, но требует наличия PostgreSQL dev-библиотек. Для разработки `psycopg2-binary` достаточен.
